...
async function _actualizarHorario(negocioId, horario) {
  const supabase = createClient();
...
// Restaurantes (modo "turno"): horario semanal organizado por turnos con nombre (Comida/Cena).
export async function actualizarHorarioTurnos(negocioId, horario) {
  return _actualizarHorario(negocioId, horario);
}

// Peluquerías / clínicas (modo "slot"): horario semanal en franjas horarias libres,
// permitiendo horario partido (varios tramos por día).
export async function actualizarHorarioSlot(negocioId, horario) {
  return _actualizarHorario(negocioId, horario);
}