...
export async function getNegociosDelUsuario(supabase) {
  ...
  const { data } = await supabase
    ...
    .select("rol, negocios ( id, nombre, tipo_negocio, config_capacidad, horario, calendarios_excepciones )")
    .eq("user_id", user?.id);
  ...
  return (data || [])
    ...
    .map((row) => ({
      ...
      horario: row.negocios.horario,
      calendarios_excepciones: row.negocios.calendarios_excepciones,
      rol: row.rol,
    }));
}
